(function(){
  ...
  function renderTags(){ if(!tagsBox) return; if(!rfids.length){ tagsBox.innerHTML=''; return; } tagsBox.innerHTML = rfids.map(r=>`<span class="badge badge-primary gap-1" data-rfid-tag="${r}">${r}<button type="button" class="ml-1" data-action="remove-rfid" data-rfid="${r}">✕</button></span>`).join(''); }
    async function fetchData(){
      if(!tbody) return;
      // Regla: si hay cualquier RFID escaneado, siempre tratamos como MULTI (no expandir a caja)
      ...
      if(hasTags){
        ...
        else { tbody.innerHTML='<tr><td colspan="9" class="text-center py-6 opacity-60">Escanea o escribe para iniciar</td></tr>'; if(status) status.textContent=''; return; }
      }
      tbody.innerHTML='<tr><td colspan="9" class="text-center py-4"><span class="loading loading-spinner loading-xs"></span></td></tr>';
      try{
        const r=await fetch(url,{headers:{'Accept':'application/json'}});
        const j=await r.json();
        if(!j.ok){ tbody.innerHTML='<tr><td colspan="9" class="text-center text-error">Error</td></tr>'; return; }
        const rows=j.items||[];
        // Si estamos en MULTI, limpiar tags que no existen en base
        if(hasTags){
